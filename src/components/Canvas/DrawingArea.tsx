...
interface DrawingAreaProps {
  ...
  // (se você já estiver passando props da “mesa de luz”, mantenha aqui)
  overlayImage?: string | null;
  imageOpacity?: number;
  imageScale?: number;
  imageOffset?: { x: number; y: number };
  imageLocked?: boolean;
  onImageOffsetChange?: (dx: number, dy: number) => void;
}
    ...
    <div className="flex-1 flex flex-col bg-background">
      ...
      <div className="border-b border-border p-3 bg-card">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            {/* NOVO: botão de imprimir (ANSI/CRLF) */}
      ...
      <div className="flex-1 overflow-auto p-4 bg-drawing-area">
        <div className="inline-block relative">
          {/* (Opcional) Imagem da mesa de luz */}
          {overlayImage && (
            <img
              src={overlayImage}
              alt="Referência"
              style={{
                ...
                pointerEvents: imageLocked ? "none" : "auto",
                userSelect: "none",
              }}
              draggable={!imageLocked}
              onDragStart={(e) => {
                if (imageLocked) return;
                // evita “arrastar imagem” padrão do browser
                e.preventDefault();
              }}
            />
          )}
          ...
          {/* Elementos de texto sobrepostos */}
      ...
      <div className="fixed bottom-0 left-0 w-full border-t border-border px-3 py-2 bg-card z-50">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          <div>Ferramenta: {selectedTool === "pencil" ? "Lápis (desenhar)" : "Selecionar"}</div>
        </div>
      </div>
    </div>
  );
